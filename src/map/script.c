...
BUILDIN_FUNC(getcostumeitem)
{
	...
	if(nameid > 0) {
		...
		{
			ShowError("buildin_getcostumeitem: este item n„o pode virar um costume.");
			return 0;
		}
...
BUILDIN_FUNC(downrefitem)
{
	...
	//Par‚metro para o n˙mero da posiÁ„o do equipamento
	num	= script_getnum(st,2);
	
	//Transforma o RID em Session Data
	sd	= script_rid2sd(st);
	
	//Checa se o personagem est· attached.
	//Caso n„o esteja, o comando p·ra aÌ.
	if (sd == NULL)
		return 0;

	//Grava a posiÁ„o do equipamento em quest„o
	if (num > 0 && num <= ARRAYLENGTH(equip))
		i = pc_checkequip(sd,equip[num-1]);
	...
	if (i > -1)
	{
		...
		//Define a reduÁ„o em -3 do item
		sd->status.inventory[i].refine = sd->status.inventory[i].refine - 3;
		
		//Status calc will happen in pc_equipitem() below
		pc_unequipitem(sd,i,3);
		
		//Realiza a reduÁ„o do refinamento em -3
		clif_refine(sd->fd,1,i,sd->status.inventory[i].refine);
		
		//Deleta o item
		clif_delitem(sd,i,1,2);
		
		//Registra o item nos Logs
		if (log_config.enable_logs&0x40)
			log_pick_pc(sd, "N", sd->status.inventory[i].nameid, 1, &sd->status.inventory[i]);
		...
		//Ativa o efeito de success/failed
		clif_misceffect(&sd->bl,2);
	}
	return 0;
}
...
/// Returns 1 if the player has a mado.
/// Otherwise, returns 0
/// checkmado();
BUILDIN_FUNC(checkmado)
{
	TBL_PC* sd;
...
/// Sets if the player has a mado or not. 
/// <flag> defaults to 1 
/// 
/// setmado <flag>; 
/// setmado; 
BUILDIN_FUNC(setmado)
{
	int flag = 1;
	TBL_PC* sd;
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(downrefitem,"i"), // [jakeRed]
	...
	BUILDIN_DEF(setmado,"?"),	// [Mara]
	BUILDIN_DEF(checkmado,""),	// [Mara]
