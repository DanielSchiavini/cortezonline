...
static unsigned short status_base_atk(const struct block_list *bl, const struct status_data *status)
{
	...
	if (bl->type == BL_PC)
		if(battle_config.renewal_setting&0x20)
			str = rstr + (dex / 5) + (status->luk / 3) + (((TBL_PC*)bl)->status.base_level / 4);
		else
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	// Creditos ao ReAM - removidas coisas in˙teis
	if(!(battle_config.renewal_setting&0x20))
	{
	...
	else //Renewal Setting
	{
		...
		if(bl->type != BL_MOB)
			status->mdef2 += status->int_ + ((status->vit + status->dex) / 5) + (level / 4);
		else	// on mobs status mdef is the same with there hard mdef
			status->mdef2 = status->mdef;

		status->status_matk = (3 * status->int_ / 2) + (status->dex / 5) + (status->luk / 3) + (level / 4);
	}

	if( bl->type&battle_config.enable_critical )
		status->cri += (short int)floor(status->luk * 0.3F + 1.0F) * 10;
	else
		status->cri = 0;
...
int status_calc_mob_(struct mob_data* md, bool first)
{
	...
	{	//Max HP setting from Summon Flora/marine Sphere
		...
		{	// different levels of HP according to skill level
			if( ud->skillid == AM_SPHEREMINE )
				status->max_hp = 400*(5 + ud->skilllv);
			else
			{ //AM_CANNIBALIZE
				status->max_hp = 10*(150 + 20*ud->skilllv + status_get_lv(mbl));
				status->mode|= MD_CANATTACK|MD_AGGRESSIVE;
			}

			status->hp = status->max_hp;
			if( ud->skillid == NC_SILVERSNIPER )
				status->rhw.atk = status->rhw.atk2 = 100*(2 + ud->skilllv);
		}
	}
	...
	{	// Strengthen Guardians - custom value +10% / lv
		...
		if(gc->castle_id < 24 || md->class_ == MOBID_EMPERIUM)
		{
			...
			{ 
				// Unconfirmed about max_hp bonus from defense investment [Inquisetor90]
				// Plant like defense does not need a boost to status->def or status->mdef.
		...
		if( md->class_ != MOBID_EMPERIUM )
		{
			status->batk += status->batk * md->guardian_data->guardup_lv / 10;
			status->rhw.atk += status->rhw.atk * md->guardian_data->guardup_lv / 10;
			status->rhw.atk2 += status->rhw.atk2 * md->guardian_data->guardup_lv / 10;
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	for( i = 0; i < EQI_MAX - 1; i++ )
	{
		...
		{
			if(battle_config.renewal_setting&0x40){
			...
			{ // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				{ //Card restriction checks.
					if( map[sd->bl.m].flag.restricted && data->flag.no_equip&(8*map[sd->bl.m].zone) )
						continue;
					if(!map_flag_vs(sd->bl.m) && data->flag.no_equip&1)if( map[sd->bl.m].flag.pvp && data->flag.no_equip & 1 )
						continue;
...
static short status_calc_aspd(struct block_list *bl, struct status_change *sc);
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	if (
		sc->data[SC_DANCING]
		|| (
			(((TBL_PC*)bl)->class_&MAPID_UPPERMASK) == MAPID_MONK && (bl->type == BL_PC) && 
...
// if(flag&SCB_RANGE)
	...
	if( flag&SCB_MATK )
	{
		...
		if ( bl->type&BL_PC && battle_config.renewal_setting&0x20){
			int wlvl = 1, wmatk = 1;
			short index = sd->equip_index[EQI_HAND_R];

			status->status_matk = status->int_ + status->int_/2 + status->dex/5 + status->luk/3 + status_get_lv(bl)/4;	
			wmatk = sd->wmatk + sd->battle_status.rhw.atk2 + sd->ematk;

			if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON)
				wlvl = sd->inventory_data[index]->wlv;

			status->matk_min = status->status_matk + wmatk;
			status->matk_max = status->status_matk + wmatk  + (wmatk * wlvl/10);
		}
...
void status_calc_bl_(struct block_list* bl, enum scb_flag flag, bool first)
{
	...
	if(bl->type == BL_PC)
	{
		...
		{
			if(!(battle_config.renewal_setting&0x4))
...
static unsigned short status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	...
	if(sc->data[SC_CONCENTRATION] && !(battle_config.renewal_setting&0x20))
...
static signed short status_calc_critical(struct block_list *bl, struct status_change *sc, int critical)
{
	...
	if(sc->data[SC_SPEARQUICKEN] && battle_config.renewal_setting&0x20)
...
static signed char status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	...
	if(sc->data[SC_KEEPING])
		return 90;
	if(sc->data[SC_STEELBODY] && !(battle_config.renewal_setting&0x20))
		return 90;
	...
	if(sc->data[SC_ASSUMPTIO] && battle_config.renewal_setting&0x20)
		def <<= 1;
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if( sc->data[SC_GT_CHANGE] )
		aspd_rate -= sc->data[SC_GT_CHANGE]->val2 * 10;
	if( sc->data[SC_GT_REVITALIZE] )
	...
	if( sc->data[SC_GLOOMYDAY] ) // CrÈdito ao brAthena
		aspd_rate += aspd_rate * sc->data[SC_GLOOMYDAY]->val3 / 100;
...
static unsigned int status_calc_maxsp(struct block_list *bl, struct status_change *sc, unsigned int maxsp)
{
	...
	if(sc->data[SC_MANDRAGORA]) //reduÁ„o de 25%+5*lv no maxsp
		maxsp += maxsp * -((25 + (5 * sc->data[SC_MANDRAGORA]->val1)) / 100); 
...
int status_get_sc_def(struct block_list *bl, enum sc_type type, int rate, int tick, int flag)
{
	...
	if( !(battle_config.renewal_setting&0x20) )
	{
		...
		case SC_ANKLE:
			if(status->mode&MD_BOSS) // Lasts 5 times less on bosses
				tick /= 5;
				sc_def = status->agi / 2; // There is no stated information that agi stat avoids ankle snare effect [Inquisetor90]
			break;
		case SC_MAGICMIRROR:
		case SC_ARMORCHANGE:
	...
	else 
	{
		...
		case SC_LEECHESEND:
			...
			//else if(sc && sc->data[SC_VENOMIMPRESS])//need info about new poison resistance.[malufett]
				//sc_def -= sc->data[SC_VENOMIMPRESS]->val2;
			else
				sc_def = 0;
			break;
		case SC_BITE:
		case SC_ELECTRICSHOCKER:
			if(!sd)
				...
				tick -= 1000*(status->agi/20);//assumed calc.[malufett]
			break;
		case SC_BURNING:
	...
	if (type == SC_ANKLE)
	{ 
		if( !(battle_config.renewal_setting&0x20) && tick < 5000) // Changed to 5 seconds according to recent tests [Playtester]
			tick = 5000;
		else if( tick < 7000)
			tick = 7000;
	}
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4, int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_EDP:	// [Celest]
			...
			tick += pc_checkskill(sd,GC_RESEARCHNEWPOISON)>0?pc_checkskill(sd,GC_RESEARCHNEWPOISON)*1000:0;
			break;
		case SC_POISONREACT:
		...
		case SC_MAGICROD:
			...
			break;
		case SC_KYRIE:
			// val4 signals it as Praefatio, which has a different # of hits formula. [LimitLine]
			val3 = ((!val4)?(val1 / 2 + 5):(6+val1)); //Hits
