...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv,int element)
{
	...
	if( sc && sc->count )
	{
		...
		if( sc->data[SC_SAFETYWALL] && (flag&(BF_SHORT|BF_MAGIC)) == BF_SHORT )
		{
			...
			if (group) {
				if (damage > group->val3 && battle_config.renewal_setting&0x20) // Wall has crumbled due to enduring too much damage.
				{
					damage -= group->val3; // Left life will reduce the damage but still receive the damage. [malufett]
					skill_delunitgroup(group);
				}else{
					if (--group->val2<=0) // Too many hits can delete the wall.
						skill_delunitgroup(group);	

					if(battle_config.renewal_setting&0x20)
						group->val3 -= damage;// Decrease life duration and block the attack
		...
		if( sc->data[SC_ASSUMPTIO] && !(battle_config.renewal_setting&0x20) && skill_num != RK_DRAGONBREATH)
		{
			if( map_flag_vs(bl->m) )
		...
		if(sc->data[SC_ENERGYCOAT] 
			&& (flag&BF_WEAPON || ( flag&BF_MAGIC && battle_config.renewal_setting&0x20) || skill_num == RK_DRAGONBREATH)// Also reduce magic damage. [malufett]
		...
		if(sc->data[SC_STEELBODY] && battle_config.renewal_setting&0x20)// [Inquisetor90]
			damage /= 10; 
...
int battle_addmastery(struct map_session_data *sd,struct block_list *target,int dmg,int type)
{
	...
	{
		case W_1HSWORD:
			if((skill = pc_checkskill(sd,AM_AXEMASTERY)) > 0 && battle_config.renewal_setting&0x20)
				damage += (skill * 3);
		case W_DAGGER:
...
static int battle_calc_base_weapon_atk(struct status_data *status, struct status_data *tstatus, short s_ele, struct weapon_atk *wa, struct status_change *sc, struct map_session_data *sd, int flag)
{
	...
	if (flag&2 && sd->state.arrow_atk)
		watk += sd->state.arrow_atk;

	weapon_atk = base_atk + ((watk * (str*100 + 20000)/200)/100 + refine + overrefine )* size_mod / 100 + status->eatk;
...
static struct Damage battle_renewal_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	if(skill_num == KN_AUTOCOUNTER || skill_num == SR_CRESCENTELBOW)
		wd.amotion >>= 1;
	...
	{	//Check for double attack.
		...
		if (sd->sc.data[SC_FEARBREEZE] && (rand()%100 < (4 + 2 * (skill_lv < 2?0:skill_lv - 2))))
		{
			int max;
			skill_lv = sd->sc.data[SC_FEARBREEZE]->val1;
			wd.div_=skill_get_num(RA_FEARBREEZE,(rand()%skill_lv)+1);
			max = wd.div_ + (sd->weapontype1==W_BOW)?2:1;
			for(i = 0; i < max; i++)
				battle_consume_ammo(sd, RA_FEARBREEZE, 0);
	...
	{	//Hit/Flee calculation
		...
		//TODO : To use db for this setting.
		if(battle_config.renewal_setting&0x10)//Renewal Settings 
		{
			int lvl_dif = status_get_lv(target) - status_get_lv(src);
			short hit = sstatus->hit;
			if(lvl_dif > 0){
				if(lvl_dif <= 5){
					hit -= hitrate * battle_config.penalty1_rate / 100;
				}else{
					hit -= hitrate * battle_config.penalty2_rate / 100;
				}
			}else{
				lvl_dif *= -1;
				if(lvl_dif <= 5){
					flee += hitrate * battle_config.penalty1_rate / 100;
				}else{
					flee += hitrate * battle_config.penalty2_rate / 100;
				}
			}
		...
		{	//Hit skill modifiers
			...
			case AS_SONICBLOW:
				...
				break;
			case GC_VENOMPRESSURE://[malufett]
				hitrate += hitrate * (10 + 4*skill_lv) / 100;
				break;
			case SC_FATALMENACE:
				hitrate -= hitrate * (35 - 5*skill_lv) / 100;
				break;
			case GN_TRAINING_SWORD:
			...
			case GN_CARTCANNON:
				if(sd && pc_checkskill(sd,GN_REMODELING_CART)>0)
					hitrate += hitrate *( 4 * pc_checkskill(sd,GN_REMODELING_CART)) / 100;
				break;
		}
		...
		if (sd && (skill = pc_checkskill(sd,GN_TRAINING_SWORD)) > 0 
			&& (sd->weapontype1 == W_DAGGER || sd->weapontype1 == W_1HSWORD) )
		...
		if(rand()%100 >= hitrate)
			...
			flag.hit = 1;
		if(sc && sc->data[SC_SPELLFIST])//ignore flee?
			flag.hit = 1;
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
			...
			case LG_SHIELDSPELL:
				...
				if (sd) {
					...
					if (index >= 0 && sd->inventory_data[index] &&
						sd->inventory_data[index]->type == IT_ARMOR)
						ATK_ADDRATE(sd->inventory_data[index]->def*280+50);
				}
				break;
			case NJ_ISSEN:
				// Calculates dmg based on atk and not str.  Sets hp to 1% of max hp. [Inquisetor90]
			...
			default:
			{
				...
				{	//Elemental attribute fix
					if( wd.damage > 0 )
					{
						...
						rand_atk = battle_attr_fix(src,target,rand_atk,s_ele,tstatus->def_ele, tstatus->ele_lv);
					...
					if( flag.lh && wd.damage2 > 0 )
					{
						...
						rand_atk2 = battle_attr_fix(src,target,rand_atk2,s_ele,tstatus->def_ele, tstatus->ele_lv);
					}
					if( sc && sc->data[SC_WATK_ELEMENT] )
					{ // Descriptions indicate this means adding a percent of a normal attack in another element. [Skotlex]
						int damage = battle_calc_base_weapon_atk(sstatus, tstatus, s_ele, &sstatus->rhw, sc, sd, (flag.arrow?2:0)) * sc->data[SC_WATK_ELEMENT]->val2 / 100;
						wd.damage += battle_attr_fix(src, target, damage, sc->data[SC_WATK_ELEMENT]->val1, tstatus->def_ele, tstatus->ele_lv);
						rand_atk += battle_attr_fix(src, target, rand_atk, sc->data[SC_WATK_ELEMENT]->val1, tstatus->def_ele, tstatus->ele_lv);

						if( flag.lh )
						{
							damage = battle_calc_base_weapon_atk(sstatus, tstatus, s_ele, &sstatus->lhw, sc, sd, (flag.arrow?2:0)) * sc->data[SC_WATK_ELEMENT]->val2 / 100;
							wd.damage2 += battle_attr_fix(src, target, damage, sc->data[SC_WATK_ELEMENT]->val1, tstatus->def_ele, tstatus->ele_lv);	
							rand_atk2 += battle_attr_fix(src, target, rand_atk2, sc->data[SC_WATK_ELEMENT]->val1, tstatus->def_ele, tstatus->ele_lv);			
						}
					}
				}
				...
				{
					//Card Fix, sd side
					if( (wd.damage || wd.damage2) && !(nk&NK_NO_CARDFIX_ATK) && !(skill_num == CR_GRANDCROSS || skill_num == NPC_GRANDDARKNESS))
					{
					...
					{ //Refine bonus applies after cards and elements.
						...
						if( index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_ARMOR ){
							...
							rand_atk += (10*sd->status.inventory[index].refine);
							rand_atk2 += (10*sd->status.inventory[index].refine);
						}
					}
				}
				if(wd.damage || wd.damage2){
					wd.damage = battle_calc_random_attack(sd, sc, sstatus, wd.damage, rand_atk, (flag.cri?1:0)|((sd) ? (sd->right_weapon.overrefine?2:0) : 0));
					if( flag.lh )
						wd.damage2 = battle_calc_random_attack(sd, sc, sstatus, wd.damage2, rand_atk2, (flag.cri?1:0)|((sd) ? (sd->left_weapon.overrefine?2:0) : 0));
				}
				...
				if(sd)
				{
					...
					if(sd->status.party_id && sc && sc->data[SC_FIGHTINGSPIRIT]){//[malufett]
						i = party_foreachsamemap(party_sub_count, sd, 0);
						if((sc->data[SC_FIGHTINGSPIRIT]->val3) >0){
							ATK_ADDRATE(7*i);//attack power equal to 7 times the number of players in your party
						}else{
							ATK_ADDRATE((7*i)*25/100);//your party members receives 1/4 of the total bonus
						}
					}
					if(sd && !skill_num && sc && sc->data[SC_EXEEDBREAK] && !sc->data[SC_SACRIFICE] && sd->equip_index[EQI_HAND_R] > 0){
						short index = sd->equip_index[EQI_HAND_R];
						int wlevel=0, wweight=sd->inventory_data[index]->weight/10;
						if (index >= 0 && sd->inventory_data[index] &&
							sd->inventory_data[index]->type == IT_WEAPON)
							wlevel = sd->inventory_data[index]->wlv;

						ATK_ADDRATE( 100 + (sc->data[SC_EXEEDBREAK]->val2+(wlevel*wweight))* status_get_lv(src)/100 );
		...
		{ // Random chance to deal multiplied damage - Consider it as part of skill-based-damage
			...
			{
				wd = battle_calc_magic_attack(src, target, sc->data[SC_SPELLFIST]->val2, sc->data[SC_SPELLFIST]->val3, wflag);
				ATK_RATE(sc->data[SC_SPELLFIST]->val1*sc->data[SC_SPELLFIST]->val3);//Spell fist bonus
		...
		else
		{
			switch( skill_num )
			{
				...
				case CR_HOLYCROSS:
					...
					if( sd && sd->status.weapon == W_2HSPEAR && battle_config.renewal_setting&0x20 )
						skillratio <<= 1;
					break;
				...
				case NPC_VAMPIRE_GIFT:
					...
					break;
				case RK_SONICWAVE:
					//29 July 2009 The skill gets a multiplicative bonus damage from base level (25% on level 150) 
					skillratio += 400 + 100*skill_lv + status_get_lv(src)/6;
					break;
				case RK_HUNDREDSPEAR: 
					//29 July 2009 The skill gets a multiplicative bonus damage from base level (25% on level 150)
					skillratio += -40 + 4*skill_lv + status_get_lv(src)/6;
					break;
				case RK_WINDCUTTER:
					//29 July 2009 The skill gets a multiplicative bonus damage from base level (50% on level 150) 
					skillratio += 50*skill_lv + status_get_lv(src)/3;
					break;
				case RK_IGNITIONBREAK:
					if(distance_bl(src,target) >= 3){
						skillratio += 100*skill_lv;
					}else if(distance_bl(src,target) == 2){
						skillratio += 100 + 200*(skill_lv-1);
					}else{//Targets closer to the player receive higher damage.
				...
				case RK_PHANTOMTHRUST:
					...
					break;
				case RK_STORMBLAST:
					skillratio += 100 * (sd?pc_checkskill(sd,RK_RUNEMASTERY):10) + 100*status_get_int(src)/4;
					break;
				case GC_CROSSIMPACT:
				...
				case RA_WUGBITE:
					...
					break;
				case NC_AXETORNADO:
					skillratio += -80 + 5*skill_lv;
					break;
				case NC_AXEBOOMERANG:
					{
						int w_weight = 0;//its says ATK% not damage.[malufett]
						if (sd) {
							short index = sd->equip_index[EQI_HAND_R];
							if (index >= 0 &&
								sd->inventory_data[index] &&
								sd->inventory_data[index]->type == IT_WEAPON)
								w_weight = sd->inventory_data[index]->weight/10;
						} else if(sd)
							w_weight = sstatus->rhw.atk2; 

						skillratio += 60 + 40*skill_lv + w_weight;
					}
					break;
				case NC_POWERSWING:
				...
				case NC_COLDSLOWER:
					...
					break;
				case NC_PILEBUNKER:
					skillratio += 100 + 50 * (skill_lv-1);
					break;
				/*case NC_ARMSCANNON:
					switch(tstatus->size){
						case 1://medium monsters.
							skillratio += 300 + (200*skill_lv-1);
							break;
						case 2://large monsters.
							skillratio += 500 + (200*skill_lv-1);
							break;
						default://small/unknown size monsters.
							skillratio += 700 + (200*skill_lv-1);
							break;
					}
					break;
				*/
				...
				case SC_FEINTBOMB:
					...
					break;
				case LG_CANNONSPEAR:
					skillratio += -50 + 50*(skill_lv-1);
					break;
				case LG_PINPOINTATTACK:
					skillratio += 100 * (skill_lv-1);
					break;
				case LG_MOONSLASHER:
					skillratio += 20 + 120*(skill_lv-1) + (sd?pc_checkskill(sd,LG_OVERBRAND)*10:0);
					break;
				case LG_OVERBRAND:
					if(skill_lv>5)
						skillratio += 200 + 300*(skill_lv-1) + (sd?pc_checkskill(sd,CR_SPEARQUICKEN)*10:0);
					else
						skillratio += 300 + 400*(skill_lv-1) + (sd?pc_checkskill(sd,CR_SPEARQUICKEN)*10:0);
					break;
				case LG_RAYOFGENESIS:
					skillratio += 200 + 300*skill_lv;
					break;
				case LG_BANISHINGPOINT:
					skillratio += -50 + 50*skill_lv + (sd?pc_checkskill(sd,SM_BASH)*10:0);//+ skill_lv==2?+10:0;is lvl2 desc typo error[110%]?
					break;
				case LG_EARTHDRIVE:
					skillratio += 100*skill_lv;
					break;
				...
				case SR_CRESCENTELBOW_AUTOSPELL:
					skillratio += 10 * tstatus->hp/1000;
					break;
				case SR_HOWLINGOFLION:
					skillratio += 80*skill_lv+100*(skill_lv-1);
					break;
				...
				case SR_KNUCKLEARROW:
					if(skill_lv > 5)
						skillratio = 150*(skill_lv-5);//+bonus??
					else
						skillratio += 100*skill_lv;
					break;
				case SR_EARTHSHAKER:
					if(skill_lv >5)
						skillratio += 200 + 150*(skill_lv-6);
					else
						skillratio += 50*(skill_lv-1);
					break;
				case SR_DRAGONCOMBO:
					skillratio += 100 + 20*(skill_lv-1) + (skill_lv>9?120:0);
					break;
				...
				case SR_GATEOFHELL:
					...
					break;
				case GN_CART_TORNADO:
					if(sd && sd->cart_weight){//They said no bonus on cart wieght, need further testing.[malufett] 
						int max_cart_weight = battle_config.max_cart_weight+5000*pc_checkskill(sd,GN_REMODELING_CART);
						skillratio += 100*sd->cart_weight/max_cart_weight; // +1% every 1% weight
					}
					skillratio += 50*skill_lv + (sd?pc_checkskill(sd,GN_REMODELING_CART)*100:500);//+100%per remodeling cart lvl
					break;
				case GN_CARTCANNON:
					skillratio += 300 + 50*(skill_lv-1) + (sd?pc_checkskill(sd,GN_REMODELING_CART):5)*status_get_int(src)/2;
					break;
				case GN_CRAZYWEED:
				...
				case SO_VARETYR_SPEAR:
					skillratio += 1000 + 400*skill_lv;
					break;
				case WM_METALICSOUND:
					skillratio += 400 + (skill_lv-1) * 50;
					break;
				case WM_GREAT_ECHO:
					skillratio += 900 + (skill_lv-1) * 100;
					break;
			}

			ATK_RATE(skillratio);

			switch(skill_num)//base lvl bonus
			{
				case GN_CART_TORNADO:
				case GN_CARTCANNON:
				case SR_GATEOFHELL:
					ATK_RATE(status_get_lv(src));
				break;
				case NC_AXEBOOMERANG:
				case NC_POWERSWING:
				case NC_AXETORNADO:
				...
				case NC_ARMSCANNON:
				case NC_FLAMELAUNCHER:
					ATK_RATE(status_get_lv(src) - 25);//lvl:150 = 1.25%;
				break;
			}
			...
			switch (skill_num) {
				...
				case RA_WUGDASH:
				case RA_WUGSTRIKE:
					ATK_ADD(sd?6*pc_checkskill(sd,RA_TOOTHOFWUG):60);//max lvl for mobs
					break;
				case NJ_ISSEN:
		...
		switch (skill_num) {
			...
			case CR_SHIELDBOOMERANG:
				if(sc && sc->data[SC_SPIRIT] &&
					...
					ATK_ADDRATE(100);
				break;
//Deal more damage if your weapon is endowed with fire element. [malufett]
			case RK_IGNITIONBREAK:
				if((sstatus->rhw.ele) == ELE_FIRE || (sstatus->lhw.ele) == ELE_FIRE){
					ATK_ADDRATE(150);//Additional damage 1.5%.
				}
//29 July 2009 The skill gets a multiplicative bonus damage from base level (25% on level 150) 
			case SO_VARETYR_SPEAR:
				ATK_ADDRATE(status_get_lv(src)/6);
				break;

			case RK_CRUSHSTRIKE:
				if(sd){
					int damage = 0, wlvl = 0, refine = 0;
					short index = sd->equip_index[EQI_HAND_R];
					if (index >= 0 &&
						sd->inventory_data[index] &&
						sd->inventory_data[index]->type == IT_WEAPON){
							wlvl = sd->inventory_data[index]->wlv;
							damage += sd->inventory_data[index]->atk; 
							refine = sd->status.inventory[index].refine;
					}

					ATK_RATE(100 + 600 * wlvl + (100 * wlvl) * refine);
				}
				break;
			case RK_PHANTOMTHRUST:
				if(sd)//Assumed additional damage +50%
					ATK_ADDRATE(pc_checkskill(sd,KN_SPEARMASTERY)*skill_lv);
				break;
			case GC_COUNTERSLASH:
			case SR_SKYNETBLOW:
				ATK_ADDRATE(sstatus->agi);
				break;
			case NC_VULCANARM:
				ATK_ADDRATE(sstatus->dex);
				break;
				break;
			case GC_CROSSRIPPERSLASHER:// damage is increased by the number of counters.[malufett]
				if(sc && sc->data[SC_ROLLINGCUTTER]){
					ATK_ADDRATE(25*sc->data[SC_ROLLINGCUTTER]->val1);//+25% each rotation.
					status_change_end(src, SC_ROLLINGCUTTER, INVALID_TIMER);//reset
				}
				break;
			case GC_VENOMPRESSURE:
				...
				break;
			case NC_POWERSWING:
				ATK_ADDRATE(sstatus->str+sstatus->dex);
				break;
			case NC_AXETORNADO:
				if((sstatus->rhw.ele) == ELE_WIND || (sstatus->lhw.ele) == ELE_WIND){
					ATK_ADDRATE(150);//Additional damage 1.5%.
				}
				ATK_ADDRATE(sstatus->vit);
				break;
			case NC_PILEBUNKER:
				ATK_ADDRATE(sstatus->str);
				break;
			case LG_RAGEBURST:
				if(sd && sd->sc.data[SC_FORCEOFVANGUARD]->val4 >0){
					ATK_ADDRATE(sd->sc.data[SC_FORCEOFVANGUARD]->val4*200);
					sd->sc.data[SC_FORCEOFVANGUARD]->val4 = 0;
					ATK_ADDRATE(status_get_lv(src));
				}
				break;
			case WM_METALICSOUND:
				if(tsc && (tsc->data[SC_SLEEP] || tsc->data[SC_DEEPSLEEP]))//also works with deep sleep?
					ATK_ADDRATE(50);
				break;
			case WM_GREAT_ECHO:
				if(sd && sd->status.party_id){
					int i=party_foreachsamemap(party_sub_count, sd, 0, JOB_WANDERER)
						+party_foreachsamemap(party_sub_count, sd, 0, JOB_WANDERER_T)
						+party_foreachsamemap(party_sub_count, sd, 0, JOB_MINSTREL)
						+party_foreachsamemap(party_sub_count, sd, 0, JOB_MINSTREL_T)
						+party_foreachsamemap(party_sub_count, sd, 0, JOB_BABY_MINSTREL)
						+party_foreachsamemap(party_sub_count, sd, 0, JOB_BABY_WANDERER);
					if(i==1)//don't include yourself
						i--;
					ATK_ADDRATE(20*i);
				}
				break;
			case SR_GENTLETOUCH_QUIET:
			case NC_BOOSTKNUCKLE:
				ATK_ADDRATE(sstatus->dex*80/100);
				break;
			case LG_PINPOINTATTACK:
				ATK_ADDRATE(sstatus->agi*50/100);//120 agi = +60%
				break;
			case SR_RAMPAGEBLASTER:
				if(sd && sd->spiritball>0)
					ATK_ADDRATE(skill_lv*50*sd->spiritball);
				break;
			case SR_FALLENEMPIRE:
				if(sd && tsd && sd->weight < tsd->weight){
					ATK_ADDRATE(tsd->weight / sd->weight *10);
				}else if(target->type == BL_MOB)
					ATK_ADDRATE(60);//+60% on mobs
				break;
			case SR_TIGERCANNON:
				ATK_ADDRATE(2000);
		...
		if (sc) {
			...
			if(sc->data[SC_ENCHANTBLADE] && !skill_num
				&& (skill_lv = sc->data[SC_ENCHANTBLADE]->val1) //[malufett]
				){
				struct Damage eb = battle_calc_magic_attack(src, target, RK_ENCHANTBLADE, skill_lv, wflag);
				ATK_ADD(eb.damage); 
			}
		}
		...
		{	//Defense reduction
			...
			// Piercing defense or flag.pdef (e.g. from ice pick or thanatos card) adds
			// (Equipment Defense/2)extra damage
			ATK_ADD2(
				flag.pdef && skill_num != MO_INVESTIGATE?(def1/2):0,
				flag.pdef2 && skill_num != MO_INVESTIGATE?(def1/2):0
			);

			// Defense formula below
			ATK_RATE2(
				flag.idef||flag.pdef?100:(4500000/(def1+450)),
				flag.idef2||flag.pdef2?100:(4500000/(def1+450))
			);
			ATK_RATE2(flag.idef||flag.pdef?100:1,flag.idef2||flag.pdef2?100:1);//little adjustment

			ATK_ADD2(
				flag.idef||flag.pdef?0:-def2,
				flag.idef2||flag.pdef2?0:-def2
			);
		...
		{	//SC skill damages
			...
			if(sc->data[SC_CARTBOOST] && 
				(skill_num == GN_CARTCANNON || skill_num == GN_CART_TORNADO || skill_num == MC_CARTREVOLUTION) )
				ATK_ADD(10*sc->data[SC_CARTBOOST]->val1);
		...
		//http://forums.irowiki.org/showthread.php?t=71037
		//Summary: Weaponary research and all those "little nevermiss bonuses from this skill, weapons forged with Star Crumbs, Spirit Spheres and so on
		// do not apply to miss cases anymore".
	...
	{	//There is a total damage value
		if(!wd.damage2)
		{
			...
			// TODO - Corrigir isso de forma menos grosseira - http://forum.cronus-emulator.com/index.php?/tracker/issue-176-temporal-de-mil-flechas-bugado/
			if( skill_num == WM_SEVERE_RAINSTORM_MELEE )
				wd.damage = (40*sd->status.base_level + sd->status.dex + sd->status.agi/5) * sd->status.base_level / 330;
		}
		else if(!wd.damage)
		{
			wd.damage2 = battle_calc_damage(src,target,&wd,wd.damage2,skill_num,skill_lv,s_ele);
			if( map_flag_gvg2(target->m) )
	...
	//When dealing non skill short range physical damage, there's a chance you will deal 3x damage with your blow. [malufett]
	if( sc && sc->data[SC_GIANTGROWTH] &&
		(sd && sd->weapontype1 >=0 && sd->weapontype1 <=8) &&
		rand() % 100 <= 10 &&//10% chance
		!skill_num ){ 
			ATK_RATE(300);
		}
	...
	if (wd.damage || wd.damage2) {
		...
		{	//Breaking of weapons. [malufett]
			int breakrate = battle_config.equip_natural_break_rate;
			if (sc) {
				if(sc->data[SC_GIANTGROWTH])
					breakrate += 1;//0.1%
			}
			if(tsc && tsc->data[SC_STONEHARDSKIN]){
					breakrate += 350;//35%
			}
			if (breakrate)
		...
		else
		{
			switch( skill_num )
			{
				...
				case KN_CHARGEATK:
					{
					int k = (wflag-1)/3; //+100% every 3 cells of distance
					skillratio += 100 * ((k > 2) ? 2 : k); // ...but hard-limited to 300%.
					}
					break;
				case HT_PHANTASMIC:
					skillratio += 50;
				...
				case NC_ARMSCANNON:
					switch( tstatus->size )
					{
						case 1: skillratio += 300 + (200*skill_lv-1);	break; // Medium
						case 2: skillratio += 500 + (200*skill_lv-1);	break; // Large
						default: skillratio += 700 + (200*skill_lv-1);	break; // Small/Unknown
					}
					if( s_level > 100 ) skillratio += skillratio * (s_level - 100) / 200;	// Base level bonus.
					break;
				case NC_AXEBOOMERANG:
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Calc base damage according to skill
			...
			default:
			{
				...
				switch( skill_num )
				{
					case MG_NAPALMBEAT:
					case MG_FIREBALL:
						if(skill_num == MG_FIREBALL && battle_config.renewal_setting&0x20)
					...
					case WZ_VERMILION:
						if(battle_config.renewal_setting&0x20){
    						int interval = 0, per, ratio=0;
    						for(per=1; per<skill_lv; per++){
     							ratio += interval; 
		...
		if(!flag.imdef){
			...
			else // Creditos ao ReAM
				if(battle_config.renewal_setting&0x20)
					ad.damage =( (ad.damage*10) * 1115 / ((mdef*-1>111?-1:1)*(1115+mdef*10)) - (mdef2*15) ) / 10;
				else
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	case MA_LANDMINE:
		if(battle_config.renewal_setting&0x20)
			md.damage=(sstatus->dex * (3+status_get_lv(src)/100) * (1+285*sstatus->int_/10000) +50+25*skill_lv-1)*skill_lv + (sd?pc_checkskill(sd,RA_RESEARCHTRAP)*40:0);
		else
			md.damage=skill_lv*(sstatus->dex+75)*(100+sstatus->int_)/100;
		break;
	case HT_BLASTMINE:
		if(battle_config.renewal_setting&0x20)
			md.damage=(sstatus->dex * (3+status_get_lv(src)/100) * (1+285*sstatus->int_/10000) )*skill_lv + (sd?pc_checkskill(sd,RA_RESEARCHTRAP)*40:0);
		else
	...
	case GN_BLOOD_SUCKER:
		...
		break;
	case GN_HELLS_PLANT_ATK:
		md.damage = (sstatus->int_ * 25) + (status_get_lv(target) * 15 * skill_lv) * (10 / (10 - pc_checkskill(sd,AM_CANNIBALIZE))) - 25; //Checar fórmula [Mara]
		break;
	}
	...
	if (!(nk&NK_IGNORE_FLEE))
	{
		...
		if(sc && sc->opt1 && sc->opt1 != OPT1_STONEWAIT)
			...
			short
				flee = tstatus->flee,
				hitrate=battle_config.renewal_setting&0x20?0:80; //Default hitrate [ReAM]
			...
			//TODO : To use db for this setting.
			if(battle_config.renewal_setting&0x10)//Renewal Settings [ReAM]
			{
				int lvl_dif = status_get_lv(target) - status_get_lv(src);
				short hit = sstatus->hit;
				if(lvl_dif > 0){
 		...
 		case RA_ICEBOUNDTRAP:
			{
				...
				wd = battle_calc_attack(BF_WEAPON,src,target,skill_num,skill_lv,md.flag);
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,int skill_num,int skill_lv,int count)
{
	...
	switch( attack_type )
	{
		case BF_WEAPON: d = (battle_config.renewal_setting&0x20) // [ReAM]
						?battle_renewal_calc_weapon_attack(bl,target,skill_num,skill_lv,count)
						:battle_calc_weapon_attack(bl,target,skill_num,skill_lv,count); break;
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "aura_lv",                            &battle_config.aura_lv,                         150,     1,     MAX_LEVEL,		},
...
// Renewal Setting [Creditos ao ReAM]
	{ "renewal_setting",                    &battle_config.renewal_setting,                 0,		0,      INT_MAX,},
...
void battle_adjust_conf()
{
	...
	if(battle_config.max_def > SHRT_MAX && !battle_config.weapon_defense_type && battle_config.renewal_setting&0x20 )
		battle_config.max_def = SHRT_MAX;

	if(battle_config.max_def > 100 && !battle_config.weapon_defense_type && !(battle_config.renewal_setting&0x20) )     // added by [Skotlex]
