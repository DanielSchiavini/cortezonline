...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	if (battle_config.display_version == 1){
		char buf[256];
		sprintf(buf, "Vers„o Cronus SVN: %s", get_svn_revision());
...
int pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	case SP_BASE_ATK:
		if(!(battle_config.renewal_setting&0x4)){
			...
			if(sd->state.lr_flag != 2) {
				bonus = status->eatk + val;
				status->eatk = cap_value(bonus, 0, USHRT_MAX);
			}
		}
		break;
	case SP_DEF1:
...
int pc_resetstate(struct map_session_data* sd)
{
	...
	{	// New statpoint table used here - Dexity
		...
		if(sd->class_&JOBL_THIRD && battle_config.renewal_setting&0x2 ){//Stat points allocation based on renewal system. [malufett]
			int i, status_point = 48, level = sd->status.base_level;
			for (i = 1; i <= (level-1); i++){
				status_point += ( (i+1) + 14) / 5 ;
				if((i+1) >=100 && (i+1) <=121)
					status_point-=2;
				else if((i+1) >121 && (i+1) <=150)
					status_point--;			
			}
	...
	case SP_BASELEVEL:
		...
		if ((unsigned int)val > sd->status.base_level) {
			...
			for (i = 1; i <= (int)((unsigned int)val - sd->status.base_level); i++){
				...
				if(sd->class_&JOBL_THIRD && battle_config.renewal_setting&0x2 ){//Stat points allocation based on renewal system. [malufett]
					int blvl = sd->status.base_level + i;
					if(blvl >=100 && blvl <=121)
						stat-=2;
					else if(blvl >121 && blvl <=150)
						stat--;
				}
			}
 ...
 *
 *------------------------------------------*/
int pc_setmado(TBL_PC* sd, int flag)
{
	if( sd->sc.data[SC__GROOMY] )
		return 0;
	...
	if( pc_checkskill( sd, NC_MADOLICENCE ) )
		pc_setoption( sd, sd->sc.option|OPTION_MADO );
	else if( pc_ismado(sd) || pc_isriding( sd, OPTION_MADO ) )
		pc_setoption( sd, sd->sc.option&~OPTION_MADO );
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if(pos & EQP_HEAD_LOW && sd->equip_index[EQI_HEAD_LOW_C] == -1) {
	...
	if(pos & EQP_HEAD_TOP && sd->equip_index[EQI_HEAD_TOP_C] == -1) {
		if(id)
	...
	if(pos & EQP_HEAD_MID && sd->equip_index[EQI_HEAD_MID_C] == -1) {
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if( sd->status.inventory[n].equip & EQP_HEAD_TOP_C )
	{
	...
	if( sd->status.inventory[n].equip & EQP_HEAD_MID_C )
	{
