...
void grave_show(time_t killtime, struct mob_data *md, char *name, struct map_session_data *sd, int oid)
{
	...
	snprintf(buffer, sizeof(buffer), "Hor疵io da Derrota: ^EE0000%s^000000", time);
	clif_scriptmes(sd, oid, buffer);

	snprintf(buffer, sizeof(buffer), "H駻oi que derrotou o MVP [^EE0000%s^000000].", name ? name : "Desconhecido");
	clif_scriptmes(sd, oid, buffer);
